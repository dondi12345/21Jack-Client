...
namespace BlackJackGame
{
    ...
    public class Game : RefSingleton<Game>
    {
        ...
        public CardsSlot playerSplitSlot;
        ...
        public void Exit()
        {
           // GamePlayController.Instance.GoToCasino();
        ...
        public void AddChipsDouble(int chip)
        {
            this.chips += chip;
            this.Money -= chip;
        ...
        IEnumerator GameLoop()
        {
            ...
            if (!this.CheckResult())
            {
                buttonsPanel.ShowButtons(true,true);
            }
        }
        ...
        void ShowWinChips(int chip, bool blackjack)
        {
            ...
            //    });
            //});
            this.winPanel.transform.DOScale(1, 0.2f).SetEase(Ease.InBack).OnComplete(() =>
            {
                this.winPanel.transform.DOScale(0, 0.2f).SetDelay(1f).SetEase(Ease.InBack);
            });
        ...
        void ShowRewards(int money)
        {
            ...
            this.rewardPanel.transform.DOScale(1, 0.2f).SetEase(Ease.InBack).OnComplete(() =>
            {
                this.rewardPanel.transform.DOScale(0, 0.2f).SetDelay(1f).SetEase(Ease.InBack).OnStart(() =>
                {
                    this.Money += money;
                });
            });
        ...
        IEnumerator StandProcess()
        {
            ...
            btnStand.interactable = false;  // ngắt tác vụ button stand
            btnHit.interactable = false;    // ngắt tác vụ button hit
            yield return BankerProcess();
            this.Final();
            btnStand.interactable = true;   // trả lại tác vụ button stand
            btnHit.interactable = true;     // trả lại tác vụ button hit
        }
        ...
        {
            SoundManager.Instance.PlayButton();
            StartCoroutine(DoubleProcess());
        }
        IEnumerator DoubleProcess()
        {
            SoundManager.Instance.PlayButton();
            Chip.Instance.SplitChipToDoubleSide();
            yield return new WaitForSeconds(1f);
            yield return HitProcess();
            yield return StandProcess();
        }
        /// <summary>
        ...
        void ShowScore(int side, bool show, int score)
        {
            ...
            if (side == 0 || side == 1)
            {
                if (!this.bankerScorePanel.alpha.Equals(show ? 1f : 0f))
                {
                    ...
                    this.bankerScorePanel.DOFade(show ? 1f : 0f, 0.2f);
                }
            }
            if (side == 0 || side == 2)
            {
                if (!this.playerScorePanel.alpha.Equals(show ? 1f : 0f))
                {
                    ...
                    this.playerScorePanel.DOFade(show ? 1f : 0f, 0.2f);
                }
            }
        }
